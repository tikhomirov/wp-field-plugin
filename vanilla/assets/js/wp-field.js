...
(function ($) {
  ...
  const WPField = {
    ...
    getFieldValue: function (fieldId) {
      ...
      if ($input.is(':radio')) {
        return (
          $('input[name="' + $input.attr('name') + '"]:checked').val() || ''
        );
      }
    ...
    addAlphaSlider: function ($field) {
      ...
      setTimeout(function () {
        ...
        const $alphaSliderInner = $(
          '<div class="iris-alpha-slider-inner"></div>'
        );
    ...
    initMediaButtons: function () {
      ...
      $(document).on(
        'click',
        '.wp-field-background-image-button',
        function (e) {
          e.preventDefault();
    ...
    openMediaFrame: function (fieldId, type, library, isEdit) {
      ...
      const self = this;
      const $wrapper = $('.wp-field-' + type + '-wrapper').has(
      ...
      frame.on('select', function () {
        ...
        if (type === 'gallery') {
          ...
          if (type === 'image') {
            let $previewWrapper = $wrapper.find(
              '.wp-field-image-preview-wrapper'
            );
            if (!$previewWrapper.length) {
              $previewWrapper = $(
                '<div class="wp-field-image-preview-wrapper"></div>'
              );
            ...
            $previewWrapper.html(
              '<img src="' +
                attachment.url +
                '" alt="" class="wp-field-image-preview">'
            );

            // Добавляем кнопку удаления если её нет
            if (!$wrapper.find('.wp-field-image-remove').length) {
              const $removeBtn = $(
                '<button type="button" class="button wp-field-image-remove" data-field-id="' +
                  fieldId +
                  '">Удалить</button>'
              );
          ...
          if (type === 'media') {
            ...
            if (attachment.type === 'image') {
              ...
              $preview.html(
                '<span class="wp-field-media-filename">' +
                  attachment.filename +
                  '</span>'
              );
            }
      ...
      selection.each(function (attachment) {
        ...
        const thumbUrl =
          data.sizes && data.sizes.thumbnail
            ...
            : data.url;
        const $item = $(
          '<div class="wp-field-gallery-item" data-id="' +
            data.id +
            '">' +
            '<img src="' +
            thumbUrl +
            '" alt="">' +
            '<span class="wp-field-gallery-remove" data-id="' +
            data.id +
            '">×</span>' +
            '</div>'
        );
      ...
      $(document).on('click', '.wp-field-repeater-remove', function (e) {
        ...
        if ($oopRow.length) {
          ...
          const count = $repeater.find(
            '.wp-field-repeater-rows > .wp-field-repeater-row'
          ).length;
    ...
    addRepeaterItem: function ($button) {
      ...
      if ($rows.length && $templateScript.length) {
        ...
        const newIndex =
          indices.length > 0 ? Math.max.apply(null, indices) + 1 : 0;
        ...
        const rowHtml = templateHtml.replace(
          /\{\{INDEX\}\}/g,
          String(newIndex)
        );
        $rows.append(rowHtml);
      ...
      const $legacyRoot = $(
        '[data-field-id="' + fieldId + '"].wp-field-repeater'
      );
      const $legacyRepeater = $legacyRoot.find('> .wp-field-repeater');
      const $legacyTarget = $legacyRepeater.length
        ? $legacyRepeater
        : $legacyRoot;

      const $template = $legacyTarget
        .find('.wp-field-repeater-template')
        .first();
      ...
      const $existingItems = $legacyTarget.find(
        '.wp-field-repeater-item:not(.wp-field-repeater-template)'
      );
      ...
      const $newItem = $template
        .clone()
        .removeClass('wp-field-repeater-template')
    ...
    checkRepeaterLimit: function ($repeater) {
      ...
      if (
        $addButton.length &&
        $repeater.find('> .wp-field-repeater-rows').length
      ) {
    ...
    initFlexibleContent: function () {
      ...
      $(document).on('click', '.wp-field-flexible-add', function (e) {
        ...
        const $layouts = $flexible.find(
          '> .wp-field-flexible-add-block > .wp-field-flexible-layouts'
        );
        const max = parseInt($flexible.data('max'), 10) || 0;
        const count = $flexible.find(
          '> .wp-field-flexible-blocks > .wp-field-flexible-block'
        ).length;
      ...
      $(document).on(
        'click',
        '.wp-field-flexible-layouts [data-layout]',
        function (e) {
          e.preventDefault();
          ...
          const newIndex =
            indices.length > 0 ? Math.max.apply(null, indices) + 1 : 0;
          const $template = $flexible
            .find(
              '.wp-field-flexible-template[data-layout="' + layoutName + '"]'
            )
            .first();
          ...
          const blockHtml = templateHtml.replace(
            /\{\{INDEX\}\}/g,
            String(newIndex)
          );
          ...
          $flexible
            .find('> .wp-field-flexible-add-block > .wp-field-flexible-layouts')
            .hide();
        }
      );
    ...
    checkFlexibleLimit: function ($flexible) {
      ...
      const $addButton = $flexible.find(
        '> .wp-field-flexible-add-block > .wp-field-flexible-add'
      );
      const $removeButtons = $flexible.find(
        '> .wp-field-flexible-blocks .wp-field-flexible-remove'
      );
    ...
    initSpinner: function () {
      $(document).on('click', '.wp-field-spinner-up', function (e) {
        ...
        const $btn = $(this);
        const $input = $btn
          .siblings('.wp-field-spinner-input-wrap')
          .find('input');
    ...
    initButtonSet: function () {
      $(document).on('change', '.wp-field-button-set input', function () {
        ...
        if ($input.is(':checked')) {
          ...
          if ($input.attr('type') === 'radio') {
            $input
              .closest('.wp-field-button-set')
              .find('.wp-field-button-set-item')
              .not($label)
              .removeClass('active');
          }
        } else {
      ...
      $('.wp-field-button-set input:checked')
        .closest('.wp-field-button-set-item')
        .addClass('active');
    },
    ...
    initAccordion: function () {
      ...
      $('.wp-field-accordion').each(function () {
        ...
        const fieldId =
          $accordion.data('field-id') ||
          'accordion_' + Math.random().toString(36).substr(2, 9);
        const savedState = localStorage.getItem(
          'wp-field-accordion-' + fieldId
        );

        // Проверяем, есть ли явно открытые элементы (open флаг)
        const hasExplicitOpen =
          $accordion.find('.wp-field-accordion-item.is-open').length > 0;
    ...
    initTabbed: function () {
      ...
      $('.wp-field-tabbed').each(function () {
        const $tabbed = $(this);
        const fieldId =
          $tabbed.data('field-id') ||
          'tabbed_' + Math.random().toString(36).substr(2, 9);
        ...
        if (savedTab) {
          ...
          const $defaultButton = $tabbed
            .find('.wp-field-tabbed-nav-item')
            .eq(defaultTabIndex);
        ...
        if (tabToActivate) {
          const $button = $tabbed
            .find('[data-tab="' + tabToActivate + '"]')
            .filter('.wp-field-tabbed-nav-item');
          if ($button.length) {
    ...
    initImageSelect: function () {
      $(document).on(
        'change',
        '.wp-field-image-select input[type="radio"]',
        function () {
          const $input = $(this);
          ...
          $container
            .find('.wp-field-image-select-item')
      ...
      $('.wp-field-image-select input[type="radio"]:checked')
        .closest('.wp-field-image-select-item')
        .addClass('selected');
    },
    ...
    initIconPicker: function () {
      ...
      $(document).on('click', '.wp-field-icon-grid span', function () {
        ...
        $input.val(iconClass);
        $button.html(
          '<span class="' + library + ' ' + iconClass + '"></span> ' + iconClass
        );
      ...
      $(document).on('input', '.wp-field-icon-search', function () {
        ...
        const $grid = $(this)
          .closest('.wp-field-icon-modal')
          .find('.wp-field-icon-grid span');
    ...
    initMap: function () {
      $('.wp-field-map').each(function () {
        ...
        const provider = String(
          $wrapper.data('map-provider') || 'google'
        ).toLowerCase();
        const zoom = parseInt($mapEl.data('zoom')) || 12;
        const centerLat =
          parseFloat($lat.val()) ||
          parseFloat($mapEl.data('center-lat')) ||
          55.7558;
        const centerLng =
          parseFloat($lng.val()) ||
          parseFloat($mapEl.data('center-lng')) ||
          37.6173;
    ...
    initPalette: function () {
      $(document).on(
        'change',
        '.wp-field-palette input[type="radio"]',
        function () {
          const $input = $(this);
      ...
      // Инициализация при загрузке
      $('.wp-field-palette input[type="radio"]:checked')
        .closest('.wp-field-palette-item')
        .addClass('selected');
    },
    ...
    initBackup: function () {
      ...
      $(document).on('click', '.wp-field-backup-validate', function (e) {
        ...
        try {
          JSON.parse(data);
          $status.html(
            '<p class="success">✓ JSON валиден и готов к импорту</p>'
          );
        } catch (e) {
