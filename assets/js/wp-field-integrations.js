(function ($) {
  ...
  function setImagePreview($wrapper, url) {
    ...
    if (url) {
      $preview.html(
        '<img src="' + url + '" alt="" class="wp-field-image-preview">'
      );
    } else {
      $preview.empty();
    }
  ...
  function bindMediaButtons() {
    ...
    $(document).on(
      'click',
      '.wp-field-file-button, .wp-field-media-button',
      function (event) {
        event.preventDefault();
        const $button = $(this);
        const isMedia = $button.hasClass('wp-field-media-button');
        const wrapperClass = isMedia
          ? '.wp-field-media-wrapper'
          : '.wp-field-file-wrapper';
        const hiddenClass = isMedia
          ? '.wp-field-media-id'
          : '.wp-field-file-id';
    ...
    $(document).on(
      'click',
      '.wp-field-gallery-add, .wp-field-gallery-edit',
      function (event) {
        event.preventDefault();
        const $button = $(this);
    ...
    $(document).on(
      ...
      function (event) {
        ...
        frame.off('select').on('select', function () {
          const attachment = frame.state().get('selection').first().toJSON();
          $('input[name="' + fieldName + '[image]"]').val(
  ...
  function bindIconPickerFallback() {
    ...
    $(document).on('click', '.wp-field-icon-button', function (event) {
      ...
      if (!$modal.length) {
        ...
        $picker
          .find('.wp-field-icon-preview')
          .attr('class', 'wp-field-icon-preview ' + next.trim());
        return;
      }
      ...
      const positionModal = function () {
        ...
        const maxHeight = Math.max(
          220,
          window.innerHeight - pickerRect.bottom - 32
        );
      ...
      if (current) {
        const $current = $modal.find(
          '.wp-field-icon-grid span[data-icon="' + current + '"]'
        );
        if ($current.length) {
          $current
            .first()
  ...
  function initMapFallback() {
    $('.wp-field-map-wrapper').each(function () {
      ...
      if (
        ...
        !$mapEl.data('leafletInit')
      ) {
        const map = window.L.map($mapEl[0]).setView(
