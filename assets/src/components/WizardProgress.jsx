...
export default function WizardProgress({ steps, activeStep }) {
  ...
  return (
    <div
      className="wp-field-wizard__progress"
      role="list"
    ...
    >
      {steps.map((step, idx) => {
        ...
        const statusClass = isDone
          ? 'wp-field-wizard__step--done'
          : isActive
            ? 'wp-field-wizard__step--active'
            : '';
        ...
        return (
          <React.Fragment key={step.id}>
            {idx > 0 && (
              <div
                className={`wp-field-wizard__step-connector${isDone ? ' wp-field-wizard__step-connector--done' : ''}`}
                aria-hidden="true"
              />
            )}
            <div
