 ...
 */
function AdminShellApp({ nav, initialSegment, initialPanel, title, sectionKey, tabKey }) {
    const leaves = collectLeaves(nav);
    ...
    const activeLeaf = findLeaf(nav, activeSegment);
    const panels = activeLeaf?.panels ?? [];
    ...
    return (
        <>
            <SidebarNav
                ...
                title={title}
            />
            {tabBarContainer && panels.length > 1 && createPortal(
                <TabBar
                    panels={panels}
